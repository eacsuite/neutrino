...
import (
	...
	"github.com/eacsuite/eacutil/gcs/builder"
	"github.com/eacsuite/eacwallet/wallet/txauthor"
	"github.com/eacsuite/eacwallet/walletdb"
	_ "github.com/eacsuite/eacwallet/walletdb/bdb"
	"github.com/eacsuite/neutrino"
	"github.com/eacsuite/neutrino/banman"
	"github.com/eacsuite/neutrino/headerfs"
)
...
func (s *secSource) add(privKey *btcec.PrivateKey) (eacutil.Address, error) {
	pubKeyHash := eacutil.Hash160(privKey.PubKey().SerializeCompressed())
	addr, err := eacutil.NewAddressWitnessPubKeyHash(pubKeyHash, s.params)
	if err != nil {
		return nil, err
	}
...
func (s *secSource) GetKey(addr eacutil.Address) (*btcec.PrivateKey, bool,
	error) {
...
func (s *secSource) GetScript(addr eacutil.Address) ([]byte, error) {
...
var (
	...
	secSrc                    *secSource
	addr1, addr2, addr3       eacutil.Address
...
func testStartRescan(harness *neutrinoHarness, t *testing.T) {
	...
	inSrc := func(tx wire.MsgTx) func(target eacutil.Amount) (
		total eacutil.Amount, inputs []*wire.TxIn,
		inputValues []eacutil.Amount, scripts [][]byte, err error) {
		ourIndex := 1 << 30 // Should work on 32-bit systems
		...
		return func(target eacutil.Amount) (total eacutil.Amount,
			inputs []*wire.TxIn, inputValues []eacutil.Amount,
			...
			inputValues = []eacutil.Amount{
				eacutil.Amount(tx.TxOut[ourIndex].Value)}
	...
	_, err = harness.h1.GenerateAndSubmitBlockWithCustomCoinbaseOutputs(
		[]*eacutil.Tx{}, rpctest.BlockVersion, time.Time{},
		[]wire.TxOut{{
			Value:    0,
			PkScript: []byte{},
		}})
...
func startRescan(t *testing.T, svc *neutrino.ChainService, addr eacutil.Address,
	...
	rescan := neutrino.NewRescan(
		...
		neutrino.NotificationHandlers(
			rpcclient.NotificationHandlers{
				...
				OnRedeemingTx: func(tx *eacutil.Tx,
					details *btcjson.BlockDetails) {
					rescanMtx.Lock()
					hash, err := chainhash.
						NewHashFromStr(
