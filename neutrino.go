...
func (sp *ServerPeer) OnFeeFilter(_ *peer.Peer, msg *wire.MsgFeeFilter) {
	...
	if msg.MinFee < 0 || msg.MinFee > eacutil.MaxSatoshi {
		log.Debugf("Peer %v sent an invalid feefilter '%v' -- "+
			"disconnecting", sp, eacutil.Amount(msg.MinFee))
		sp.Disconnect()
		return
	}
